...
constexpr int FP4_PER_INT8 = 2;  // 2 FP4 values per int8
...
__device__ __forceinline__ uint8_t fp8_to_fp4(__nv_fp8_e4m3 fp8_val, float scale) {
    float val = __fp8_to_float(fp8_val) / scale;
...
__device__ __forceinline__ __nv_fp8_e4m3 fp4_to_fp8(uint8_t fp4_val, float scale) {
    ...
    return __float_to_fp8_e4m3(val);
}

// Pack 16 FP8 values into 16 FP4 (8 INT8) values + 1 FP8 scale
__device__ __forceinline__ void pack_fp8_group_to_fp4(
    const __nv_fp8_e4m3* fp8_values,
    uint8_t* packed_data,
    int group_offset
) {
...
__device__ __forceinline__ void unpack_fp4_group_to_fp8(
    const uint8_t* packed_data,
    __nv_fp8_e4m3* fp8_values,
    int group_offset
) {
    ...
    float scale = __fp8_to_float(fp8_scale);
...
__global__ void compute_and_compress_fp8_kv_cache(
    ...
    __nv_fp8_e4m3 k_fp8 = __float_to_fp8_e4m3(input_k[tid]);
    __nv_fp8_e4m3 v_fp8 = __float_to_fp8_e4m3(input_v[tid]);
...
void decompress_fp4_cache_to_fp8_forward_wrapper(
    ...
    TORCH_CHECK(compressed_k_cache.dtype() == torch::kUInt8, "compressed_k_cache must be uint8");
    TORCH_CHECK(compressed_v_cache.dtype() == torch::kUInt8, "compressed_v_cache must be uint8");
    TORCH_CHECK(k_fp8_output.dtype() == torch::kFloat8E4M3FN, "k_fp8_output must be fp8");
    TORCH_CHECK(v_fp8_output.dtype() == torch::kFloat8E4M3FN, "v_fp8_output must be fp8");
